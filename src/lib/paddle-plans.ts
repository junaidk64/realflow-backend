...
export const SEEDED_WORKFLOWS = [
  {
    workflowKey:   "wf-001",
    n8nWorkflowId: "realflow-wf-001",
    name:          "New Lead Follow-Up Sequence",
    description:   "Automatically sends a follow-up email and SMS when a new lead is created.",
  ...
  },
  {
    ...
    description:   "Sends automated reminders to leads 24 hours before a scheduled appointment.",
  ...
  },
  {
    ...
    description:   "Notifies matched leads when a new property listing goes live.",
  ...
  },
  {
    ...
    description:   "Generates and emails a monthly performance summary for your agency.",
...
export function canEnableWorkflow(userPlan: string, workflowKey: string): boolean {
...
export function getAllowedWorkflows(plan: string): string[] {
  return SEEDED_WORKFLOWS
    .filter((wf) => canEnableWorkflow(plan, wf.workflowKey))
