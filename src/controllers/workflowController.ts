...
export const getWorkflows = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const workflows = await Workflow.find({ userId: req.user!.userId }).sort({ createdAt: -1 });
...
export const createWorkflow = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { name, description, type, webhookUrl, config: wfConfig, n8nWorkflowJson } = req.body;
...
export const getWorkflowTemplates = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const getWorkflowExecutions = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    const workflow = await Workflow.findOne({ _id: id, userId: req.user!.userId });
    ...
    const executions = await getWorkflowExecutions(workflow.n8nWorkflowId);
    res.json({ success: true, data: { executions } });
  } catch (error) {
    next(error);
  }
