...
export const handleGmailWebhook = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const body = req.body;
    ...
    if (body.message?.data) {
      try {
        ...
        if (emailAddress) {
          const connection = await GmailConnection.findOne({ email: emailAddress, isActive: true });
          if (connection) {
            await addEmailProcessingJob(
              (connection.userId as string).toString(),
              (connection._id as string).toString()
            );
...
export const handleN8nWebhook = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const body = req.body;
