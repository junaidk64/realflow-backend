	...
	try {
		...
		res.json({
			...
			data: {
				...
				dailyLeads,
				conversionRate:
					total > 0 ? Math.round(((statusMap.won || 0) / total) * 100) : 0,
			},
		})
	} catch (error) {
		next(error)
		...
		const csvRows = [
			...
			...leads.map((l) => {
				const extra = l.extraFields
					? Object.values(l.extraFields as Record<string, unknown>)
							.filter(Boolean)
							.join('; ')
					: ''
				return [
...
export const createLeadFromN8n = async (
	req: Request,
	res: Response,
	next: NextFunction,
): Promise<void> => {
...
// Runs the exact same pipeline as a real incoming Gmail email —
// spam filter → AI extraction → lead creation → auto-reply → n8n triggers.
// Only difference: email data comes from the request body instead of Gmail.
export const testClassifyEmail = async (
	req: Request,
	res: Response,
	next: NextFunction,
): Promise<void> => {
	try {
		...
		// Find active Gmail connection — needed by the auto-reply worker downstream
		const gmailConnection = await GmailConnection.findOne({
			userId,
			isActive: true,
		})

		// Create EmailLog exactly as emailProcessingWorker does for real emails
		...
		// Push into the same lead extraction queue — all downstream workers run normally
		const job = await leadExtractionQueue.add('extract-lead', {
			userId,
			gmailConnectionId: gmailConnection?._id?.toString() ?? null,
		...
		logger.info(`Test email queued for user ${userId}, job ${job.id}`)

		res.json({
			success: true,
			message:
				'Email queued — full pipeline will run (spam filter → AI extraction → lead creation → auto-reply → n8n triggers) based on your active workflows',
			jobId: job.id,
			emailLogId: emailLog._id,
			gmailConnectionFound: !!gmailConnection,
		})
	} catch (error) {
		next(error)
	}
