...
// Gemini 1.5 Flash — free tier, 1M token context, fast
...
- spam: newsletters, promotions, automated notifications, no human intent
- simple: short direct enquiry, single clear request, under ~200 words
- complex: long thread, multiple questions, ambiguous intent, or needs context
...
Reply with exactly one word: spam, simple, or complex`
    ...
    logger.warn(`Gemini classify failed, defaulting to simple: ${(err as Error).message}`)
    return 'simple'
  }
}
...
Write the summary now:`
    ...
    logger.warn(`Gemini summarize failed, using truncation: ${(err as Error).message}`)
    return thread.slice(0, 1200)
  }
}
