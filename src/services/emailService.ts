...
function getBusinessDetails(lead: Partial<ILead>): DetailItem[] {
	...
	switch (type) {
		case 'moving':
			items.push(
				lead.fromAddress
					? { icon: '📍', label: 'Moving From', value: lead.fromAddress }
					: null,
				lead.toAddress
					? { icon: '🏠', label: 'Moving To', value: lead.toAddress }
					: null,
				lead.movingDate
					? { icon: '📅', label: 'Moving Date', value: lead.movingDate }
					: null,
				lead.services?.length
					? { icon: '📦', label: 'Services', value: lead.services.join(', ') }
					: null,
			)
			break
		case 'real_estate': {
		...
		case 'insurance': {
			...
			items.push(
				...
				coverage
					? { icon: '💰', label: 'Coverage Amount', value: coverage }
					: null,
				renewal ? { icon: '📅', label: 'Renewal Date', value: renewal } : null,
				provider
					? { icon: '🏢', label: 'Current Provider', value: provider }
					: null,
			)
			break
		}
		case 'cleaning': {
			...
			items.push(
				...
				propType
					? { icon: '🏠', label: 'Property Type', value: propType }
					: null,
				freq ? { icon: '🔄', label: 'Frequency', value: freq } : null,
				sqft
					? { icon: '📐', label: 'Area Size', value: `${sqft} sq ft` }
					: null,
			)
			break
		}
		case 'legal': {
			...
			items.push(
				...
				consult
					? { icon: '📅', label: 'Consultation Date', value: consult }
					: null,
			)
			break
		}
		default: {
			...
			items.push(
				service
					? { icon: '🔧', label: 'Service Required', value: service }
					: null,
				prefDate
					? { icon: '📅', label: 'Preferred Date', value: prefDate }
					: null,
				budget ? { icon: '💰', label: 'Budget', value: budget } : null,
			)
		}
		...
		case 'moving':
			return {
				...
				steps: [
					{
						title: 'Review',
						desc: 'We review your moving requirements in detail',
					},
					{
						title: 'Quote',
						desc: 'We prepare a competitive, transparent quote',
					},
					{
						title: 'Contact',
						desc: "We'll reach out within 2 hours to confirm",
					},
				],
			}
		case 'real_estate':
			return {
				...
				steps: [
					{
						title: 'Review',
						desc: 'We review your property preferences carefully',
					},
					{
						title: 'Match',
						desc: 'We identify properties that fit your needs',
					},
		...
		case 'insurance':
			return {
				...
				steps: [
					...
					{
						title: 'Compare',
						desc: 'We compare top policies from leading providers',
					},
					{
						title: 'Advise',
						desc: 'Our advisor will present the best options',
					},
				],
			}
		case 'cleaning':
			return {
				...
				steps: [
					...
					{
						title: 'Schedule',
						desc: 'We check availability for your preferred date',
					},
					{
						title: 'Confirm',
						desc: "We'll confirm your booking with full details",
					},
				],
			}
		case 'legal':
			return {
				...
				steps: [
					...
					{
						title: 'Advise',
						desc: 'We provide expert legal guidance and next steps',
					},
				],
			}
		default:
...
${renderDetailRows(details)}
	...
	const resolvedAutoReplyTemplate = settings?.autoReplyTemplate
		? settings.autoReplyTemplate.replace(
				/\{\{(\w+)\}\}/g,
				(_, key) => autoReplyTemplateVars[key] ?? '',
			)
		: ''
...
export const sendAutoReply = async (
	...
	if (userId) {
		...
		if (smtpConn) {
			try {
				...
				logger.info(
					`Auto-reply sent via SMTP to ${toEmail}, messageId: ${info.messageId}`,
				)
				return { success: true, messageId: info.messageId, html: htmlContent }
			} catch (err) {
				logger.warn(
					`SMTP auto-reply failed for user ${userId}, falling back to Gmail: ${(err as Error).message}`,
				)
			}
		}
	}
	...
	try {
		...
		logger.info(
			`Auto-reply sent via Gmail to ${toEmail}, messageId: ${response.data.id}`,
		)
		return {
			success: true,
...
export const sendEmailForUser = async (
	...
	if (smtpConn) {
		try {
			...
			logger.warn(
				`SMTP send failed for user ${userId}, falling back to Gmail: ${(err as Error).message}`,
			)
		}
	}
	...
	if (gmailConn) {
		try {
			...
			return {
				success: true,
				messageId: resp.data.id || undefined,
				provider: 'gmail',
			}
		} catch (err) {
			logger.error(
				`Gmail send failed for user ${userId}: ${(err as Error).message}`,
			)
			return {
				success: false,
	...
	return {
		success: false,
		error:
			'No email provider configured. Connect SMTP or Gmail in your account settings.',
		provider: 'none',
	}
}
