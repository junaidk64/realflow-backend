...
const createOAuth2Client = (accessToken: string, refreshToken: string): OAuth2Client => {
...
export const setupGmailWatch = async (gmailConnection: IGmailConnection): Promise<void> => {
  try {
    const auth = await getAuthenticatedClient(gmailConnection);
    const gmail = google.gmail({ version: 'v1', auth });

    if (!config.google.pubsubTopic) {
      logger.warn('GOOGLE_PUBSUB_TOPIC not configured, skipping Gmail watch setup');
      return;
    }
    ...
    logger.info(`Gmail watch setup for ${gmailConnection.email}, expires: ${expiry}`);
  } catch (error) {
    logger.error(`Failed to setup Gmail watch for ${gmailConnection.email}:`, error);
    throw error;
  }
};
...
export const processNewEmails = async (
  ...
  try {
    ...
    if (gmailConnection.historyId) {
      try {
        ...
        if ((historyError as { code?: number }).code === 404) {
          ...
          logger.warn(`History not found for ${gmailConnection.email}, falling back to list`);
          messageIds = await listMessages(gmailConnection, 10, 'in:inbox is:unread newer_than:1d');
        } else {
          throw historyError;
        }
      }
    ...
    logger.error(`Failed to process emails for ${gmailConnection.email}:`, error);
    await GmailConnection.findByIdAndUpdate(gmailConnection._id, {
